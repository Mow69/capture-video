...
def get_filter_path(id_filter):
    for filter in all_filter_json:
        if(filter["id"] == id_filter):
            return filter["path"]
    return
...
actual_filter_index = all_filter_json[0]["id"]
...
s = BluetoothServer(data_b)

def random_filter_function():
    random_int = random.randint(0,len(all_filter_json)-1)
    return all_filter_json[random_int]["id"]
...
def check_app_commande():
   ...
   #________________________________PAUSE___________________________________________________________
    ...
    if app_commande_function == 'random':
        ...
        cap_filter.release()
        retFilter, cap_filter = initDevice.init_filter(get_filter_path(random_filter_function()))
...
while True:
    ...
    if retFilter == False: #reload filter video and remove cache
        if is_random :
           retFilter, frame_filter, cap_filter = initDevice.reload_filter(get_filter_path(random_filter_function()),cap_filter)  
        else :
